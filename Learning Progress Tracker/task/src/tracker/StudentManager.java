...
public class StudentManager {
    public Map<String, Student> map = new LinkedHashMap<>();
    ...
    public void listStudentCommand() {
        if (map.isEmpty()) {
    ...
    private void updatePoints(String id, String pointString) {
        ...
        for (int i = 0; i < points.length; i++) {
            if (points[i] > 0) {
                ...
                COURSES[i].addSubmission(); // the times of submission of all students
                COURSES[i].totalPoint(points[i]); // total point of all students
    ...
    public void findStudentCommand() {
        Scanner scanner = new Scanner(System.in);
